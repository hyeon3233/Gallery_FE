...
const submit = async () => {
  if (state.form.payment !== 'card') {
    // 결제수단이 카드가 아니라면 카드번호를 지운다
    state.form.cardNumber = '';
  }
  // 구매하고자 하는 item pk값을 넣어야함.
  ...
  if (state.form.payment === 'bank') {
    ...
    message.push(`한국은행 123-456-777 계좌로 ${price}원을 입금해주세요`);
  }
      ...
      <div class="row g-5">
        <div class="col-md-5 col-lg-4 order-md-last">
          <div class="mb-3">
            ...
            <span class="badge bg-primary rounded-pill align-middel">
              {{ state.items.length }}
            </span>
          </div>
          ...
          <div class="my-3">
            ...
            <div class="form-check">
              <input
                ...
                v-model="state.form.payment"
              />
              <label for="bank" class="form-check-label">무통장 입금</label>
            </div>
          </div>
